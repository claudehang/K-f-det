...
hist = model.fit(train_tensors, train_targets, batch_size=32, epochs=50, 
        validation_data=(valid_tensors, valid_targets), callbacks=[checkpointer, tbCallback],
        verbose=2)
# model.save_weights('stage_one_weights.h5')
...
# model.load_weights('stage_one_weights.h5')
model.summary()
hist_s2 = model.fit(train_tensors, train_targets, batch_size=32, epochs=100, 
        validation_data=(valid_tensors, valid_targets), callbacks=[checkpointer, tbCallback],
        verbose=2)
...
plt.plot(hist.history['acc']+hist_s2.history['acc'])
plt.plot(hist.history['val_acc']+hist_s2.history['val_acc'])
...
plt.figure(figsize=(8, 5), dpi=100)
plt.plot(hist.history['loss']+hist_s2.history['loss'])
plt.plot(hist.history['val_loss']+hist_s2.history['val_loss'])
