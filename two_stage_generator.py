...
hist_stage1 = model.fit_generator(generator=training_generator, validation_data=validation_generator, use_multiprocessing=True, workers=6, 
                                  callbacks=[checkpointer, tbCallback], epochs=80, verbose=2)

print("======  Starting another stage of training!  ======")
# model.load_weights('stage_one_weights.h5')
for layer in base_model.layers:
    layer.trainable = True
model.compile(optimizer=RMSprop(lr=0.000001), loss='categorical_crossentropy', metrics=['accuracy'])
# model.load_weights('stage_one_weights.h5')
model.summary()
hist_stage2 = model.fit_generator(generator=training_generator, validation_data=validation_generator, use_multiprocessing=True, workers=6, 
                                  callbacks=[checkpointer, tbCallback], epochs=60, verbose=2)


# summarize history for accuracy
plt.figure(figsize=(10, 5), dpi=100)
plt.plot(hist_stage1.history['acc']+hist_stage2.history['acc'])
plt.plot(hist_stage1.history['val_acc']+hist_stage2.history['val_acc'])
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train', 'validate'], loc='upper left')
# plt.show()
plt.savefig('result-show/training_acc.png', dpi=300)


# summarize history for loss
plt.figure(figsize=(10, 5), dpi=100)
plt.plot(hist_stage1.history['loss']+hist_stage2.history['loss'])
plt.plot(hist_stage1.history['val_loss']+hist_stage2.history['val_loss'])
plt.title('model loss')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'validate'], loc='upper left')
# plt.show()
plt.savefig('result-show/training_loss.png', dpi=300)
